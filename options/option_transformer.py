...
def get_args_parser():
    ...
    parser.add_argument('--mask-only-motion-tokens', action='store_true', help='Causal mask only motion tokens when doing self-attention')
    parser.add_argument('--cond-mask-prob', type=float, default=0.0, help='Conditional drop probability')
    parser.add_argument('--debug-target-type-list', choices=['angle', 'distance', 'relX', 'relY', 'relZ', 'relV', 'ground'], nargs='+', default=None, help='Target type list for debugging Category Aware Loss')
    parser.add_argument('--mask-method', type=str, default='vanilla', choices=['vanilla', 'symmetric_distance', 'confidence_based_symmetric_distance'], help='Masking method')
    parser.add_argument('--max-gamma', type=float, default=0.5, help='Masking gamma value')
    parser.add_argument('--min-gamma', type=float, default=0.5, help='Masking gamma value')
    parser.add_argument('--init-prior', action='store_true', help='Initialize the bias of the output layer based on the prior distribution of the training data')
    parser.add_argument('--gamma-schedule', default='linear', choices=['linear', 'random', 'fix'], help='')
    # parser.add_argument("--loss-name", type=str, default=None, choices=[None, 'cace'], help='cace:Catogory Aware Cross Entropy Loss')
    parser.add_argument('--froze-codebook', action='store_true', help='Whether to froze the codebook during transformer training')
    parser.add_argument('--text-encoding-method', default='baseline', choices=['baseline', 'graph_reasoning'], help='Whether to use graph-based reasoning text encoder')
    parser.add_argument('--block-attend-cond2cond', action='store_true', help='Whether to block attention between condition tokens in condition-to-condition attention')
