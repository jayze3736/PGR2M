...
class Coder():
    def __init__(self, dataname):
        ...
        self.smpl2kit = {0:0,1:16,2:11,3:1,4:17,5:12,6:2,7:19,8:14,9:3,10:20,11:15,12:4,13:None,14:None,15:None,16:5,17:8,18:6,19:9,20:7,21:10}
        # Joints that are actually useful for the captioning pipeline
        self.virtual_joints = ["left_hand", "right_hand", "torso"] # ADD_VIRTUAL_JOINT
        self.joint_names = self.all_joint_names[:22] + ['left_middle2', 'right_middle2'] + self.virtual_joints
        self.joint_names2id = {jn:i for i, jn in enumerate(self.joint_names)}

        self.interpretation_set = flatten_list([p["category_names"] for p in POSECODE_OPERATORS_VALUES.values()])
        sp_interpretation_set = [v[1][1] for v in SUPER_POSECODES if v[1][1] not in self.interpretation_set]
        self.interpretation_set += self.list_remove_duplicate_preserve_order(sp_interpretation_set) # 
        self.intptt_name2id = {intptt_name:i for i, intptt_name in enumerate(self.interpretation_set)}

        self.all_joint_names2id = {jn:i for i, jn in enumerate(self.all_joint_names)}

    def flatten_list(self, l):
        return [item for sublist in l for item in sublist]

    def list_remove_duplicate_preserve_order(self, seq):
        seen = set()
        seen_add = seen.add
        return [x for x in seq if not (x in seen or seen_add(x))]

    def format_kit(self, joints):
        formatted = np.zeros((joints.shape[0], 22, 3))
        for i in range(22):
            if self.smpl2kit[i] is None:
                continue
            formatted[:,i,:] = joints[:,self.smpl2kit[i],:]
        return formatted
    ...
    def motion2poseqs(self, coords, verbose=False, device=None):
        ...
        p_queries = self.prepare_posecode_queries() 

        sp_queries = {} 

        coords = self.prepare_input(coords) 
        ...
        new_vec_formatted = np.zeros((new_vec.shape[0],len(id_to_name))) # 0 base vector
        ...
        new_vec_formatted = new_vec_formatted.astype(bool)  

        return new_vec_formatted
    ...
    def compute_wrist_middle2ndphalanx_distance(self, coords):
        x = distance_between_joint_pairs([
            [self.all_joint_names2id["left_middle2"], self.all_joint_names2id["left_wrist"]],
            [self.all_joint_names2id["right_middle2"], self.all_joint_names2id["right_wrist"]]], coords)
        return x.mean().item()

    def prepare_input(self, coords):
        """
        ...
        if nb_joints == 22:
            ...
            left_v = coords[:,self.all_joint_names2id["left_wrist"]] - coords[:,self.all_joint_names2id["left_elbow"]]
            right_v = coords[:,self.all_joint_names2id["right_wrist"]] - coords[:,self.all_joint_names2id["right_elbow"]]
            # new phalanx coordinate
            added_j = [x*left_v/torch.linalg.norm(left_v, axis=1).view(-1,1) \
                            + coords[:,self.all_joint_names2id["left_wrist"]],
                        x*right_v/torch.linalg.norm(right_v, axis=1).view(-1,1) \
                            + coords[:,self.all_joint_names2id["right_wrist"]]]
        ...
        if nb_joints >= 52:
            ...
            keep_joints_indices = [self.all_joint_names2id[jn] for jn in self.joint_names[:-len(self.virtual_joints)]]
            coords = coords[:,keep_joints_indices]
        ### add virtual joints
        added_j = [0.5*(coords[:,self.joint_names2id["left_wrist"]] + coords[:,self.joint_names2id["left_middle2"]]), # left hand
                    0.5*(coords[:,self.joint_names2id["right_wrist"]] + coords[:,self.joint_names2id["right_middle2"]]), # right hand
                    1/3*(coords[:,self.joint_names2id["pelvis"]] + coords[:,self.joint_names2id["neck"]] + coords[:,self.joint_names2id["spine3"]]), # torso
                    # ADD_VIRTUAL_JOINT
                    ]
        added_j = [aj.view(-1, 1, 3) for aj in added_j]
        coords = torch.cat([coords] + added_j, axis=1) # concatenate along the joint axis
    ...
    def prepare_posecode_queries(self):
        """
        ...
        for posecode_kind, posecode_list in ALL_ELEMENTARY_POSECODES.items():
            ...
            joint_ids = torch.tensor([[self.joint_names2id[jname] for jname in p[0]]
                                        if type(p[0])!=str else self.joint_names2id[p[0]]
    ...
    def infer_posecodes(self, coords, p_queries, sp_queries, verbose = True, device=None):
        ...
        for p_kind, p_operator in POSECODE_OPERATORS.items():
            ...
            if device is not None:
                rand = torch.rand(val.shape).to(device)
                val = val.to(device)
                val += (rand*2-1) * p_operator.random_max_offset 
            else:
    ...
    def infer_attribute_value(self, coords, p_queries, verbose = True, device=None):
        ...
        for p_kind, p_operator in POSECODE_OPERATORS.items():
            ...
            if device is not None:
                rand = torch.rand(val.shape).to(device)
                aug_val = aug_val.to(device)
                aug_val += (rand*2-1) * p_operator.random_max_offset
            else:
                aug_val += (torch.rand(val.shape)*2-1) * p_operator.random_max_offset

            p_value.append(val)
            p_aug_value.append(aug_val)

        eval_value = torch.concat(p_value, dim=1)
        eval_aug_value = torch.concat(p_aug_value, dim=1)

        return eval_value, eval_aug_value

    def motion2posval(self, coords, verbose=False, device=None):
        ...
        coords = self.prepare_input(coords)
        eval_value, eval_aug_value = self.infer_attribute_value(coords, p_queries, verbose=verbose, device = device)
