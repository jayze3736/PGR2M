...
def main(coords, verbose=True, file_path = None):
...
def prepare_input(coords):
    ...
    if nb_joints == 22:
        ...
        added_j = [x*left_v/torch.linalg.norm(left_v, axis=1).view(-1,1) \
                        + coords[:,ALL_JOINT_NAMES2ID["left_wrist"]], # left_middle2
                    x*right_v/torch.linalg.norm(right_v, axis=1).view(-1,1) \
                        + coords[:,ALL_JOINT_NAMES2ID["right_wrist"]]] # right_middle2
...
if __name__ == "__main__" :
    ...
    for filename in tqdm(findAllFile(args.in_dir)): # npy 데이터
        ...
        f = open(filename,"rb")
        if args.dataname == 'kit':
            pred_xyz = np.load(f).reshape((-1,21,3))
        else: # t2m
        ...
        output_path = filename.replace("new_joints", "codes_v2")
        ...
        # pred_xyz
        main(pred_xyz, file_path = output_path, verbose=False)
